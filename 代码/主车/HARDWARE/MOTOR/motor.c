...
if(yaw<=90)
				{ 
					flag_inc=31;
				 MotorALL_Ctrl(-speed,-speed,+1.5*speed,+1.5*speed); 
				}	
if(yaw>90)
{ 
    flag_inc=100;						
...
if(yaw<=180&&yaw>0)
				{ 
					flag_inc=37;
				 MotorALL_Ctrl(-1*speed,-1*speed,+1.5*speed,+1.5*speed); 
				}	
if(yaw<0)
{ 
    flag_inc=100;						
...
if(yaw>=0)
				{ 
					flag_inc=51;
				 MotorALL_Ctrl(-1.5*speed,-1.5*speed,+1*speed,+1*speed); 
				}	
if(yaw<0)
{ 
    flag_inc=100;						
...
if(yaw>0)
	...
	{		
MotorALL_Ctrl(6500,6500,-5000,-5000);	
	}	
if(yaw<-90)
...
if(yaw<0)			
{
	MotorALL_Ctrl(-2000,-2000,2000*1.2,2000*1.2); 
}
if(yaw>=90)
				{ 					
	   flag_inc=57;	
MotorALL_Ctrl(-1.3*speed,-1.3*speed,+0.8*speed,+0.8*speed); 					
        }		
if(yaw<90&&yaw>0)
				{ 					
	   flag_inc=100;						
...
if(yaw>0)
				{ 
				flag_inc=54;
				 MotorALL_Ctrl(-speed,-1*speed,+1.5*speed,+1.5*speed); 
				}
if(yaw<=0)
				{ 					
	   flag_inc=100;						
...
if(yaw>0)
				{ 
				flag_inc=55;
				 MotorALL_Ctrl(-1.5*speed,-1.5*speed,+speed,+speed); 
				}
if(yaw<=0)
				{ 					
	   flag_inc=100;						
...
//				OLED_Show3FNum(0,0,distance_average ,4,2,12);	
//					OLED_Refresh_Gram();//更新显示到OLED	
				if(xunji_L_1==1&&xunji_L_2==1&&xunji_F==1&&xunji_R_1==1&&xunji_R_2==1)
		    { 
//					flag_inc=100;
					c1=0;
					c2=0;
...
void go_back_5(int a,int b)//向后直行
{     
      speed_bais=5000;
	    velocity_decrease=160;	
	PID_weizhi.integral=0;	
